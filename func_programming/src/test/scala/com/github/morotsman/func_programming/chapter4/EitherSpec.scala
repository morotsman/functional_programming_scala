...
class EitherSpec  extends FlatSpec with Matchers{
  ...
  "Either.sequence2(List(Either.Try(4/2), Either.Try(8/2), Either.Try(10/2)))" should "result in Right(List(2,4,5))" in {
    assert(Either.sequence2(List(Either.Try(4/2), Either.Try(8/2), Either.Try(10/2)))  == Right(List(2,4,5)))
  }  
  
  "Either.sequence2(List(Either.Try(4/2), Either.Try(8/2), Either.Try(10/2)))" should "result in Left(java.lang.ArithmeticException)" in {
    assert(isLeft(Either.sequence2(List(Either.Try(4/0), Either.Try(8/2))))  == true)
  }   
  
  "Either.traverse(List(1, 2, 3))" should "result in Right(List(2,4,5))" in {
    assert(Either.traverse(List(1, 2, 5))(v => Either.Try(10/v))  == Right(List(10,5,2)))
  }   
  
  "Either.traverse(List(1, 0, 3))" should "result in Left(java.lang.ArithmeticException)" in {
